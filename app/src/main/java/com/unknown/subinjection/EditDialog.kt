...
class EditDialog(context:Context,val sub:String,val main: Main):AlertDialog(context) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.editBtn.setOnClickListener {
            val configApi = "$baseUrl/editor/replace"
            val params = JSONObject()
            params.put("sign", crypto.enc(sign))
            params.put("oldSub", crypto.enc(sub))
            params.put("sub", crypto.enc(binding.sub.text.toString()))
            val request = object : JsonObjectRequest(
                Method.POST, configApi, params,
                Response.Listener {
                    dismiss()
                    Toast.makeText(context, "Sub was edited.", Toast.LENGTH_SHORT).show()
                    main.subEdited()
                },
